...
#define PLAY_CREATE_TIMER_ERROR		11		// create playtimer failed
...
#define PLAY_OPEN_AUDIO_ENCODER_ERROR	  26 // open audio encoder failed
#define PLAY_NEED_LARGER_BUFFE			  27 // need large buffer
...
typedef enum
{
	...
	DECODE_HW_TEXTURE,			// Android surfaceTexure
	DECODE_HW_NV_CUDA,			// CUDA
	DECODE_HW_D3D11,			// GPU CopyBack via dx11,only windows platform
}DecodeType;
...
/* PLAY_CMD_GetRenderTime query struct*/
typedef struct _RenderTime
{
	...
	int tm_millisec;  /* milliseconds after the second - [0,1000] */
}RenderTime;

typedef enum
{
	AV_SYNC_VIDEO_MAIN,		  // based in video(no audio and video synchronization)
...
typedef struct _FILE_INFO
{
	...
	char			serial_number[20];		// NVR serial number
	UINT			rev[4];					// Reserved
...
typedef struct
{
	...
	int				nIfMeetGB28181;			// if meet GB28181, 0 to yes, others are errors
	int				nGopBitRate;			// Gop bit rate(kbps), 0 means it's still calculating

	int				nTotalChannel;			//apply to audio muti channels 
	int				nCurChannel;			//apply to audio muti channels,current channel

	int				nReserved[54];			//Reserved
}FRAME_INFO_EX;

typedef struct
{
	unsigned char nColorPrimaries;    //视频三原色类型
	unsigned char nColorTransfer;     //视频信号传输类型
	unsigned char nColorSpace;       //颜色空间
	unsigned char bColorFullRange;   //颜色范围，0：16-235；1：0-255
}ColorFormat;
...
typedef struct 
{
	...
	LONG			nType;					// Video frame type,T_AUDIO16,T_RGB32,T_IYUV
    unsigned int	nTimeStamp;		        // Time stamp
	int				nDecodeChannel;				
	ColorFormat     nColorFormat;
	int				nReserved[56];				// 保留字段
}FRAME_DECODE_INFO;
 ...
 *		close previous audio if current audio is enabled. 
 *		Please note audio is closed by default
 *
 * @param[in] nPort Port number
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
...
/**
 * Close audio in share way. PLAY_PlaySound and PLAY_StopSound are enable audio in 
 *		exclusive way. Please note, in one progress, all channels are required to use
 *		same way to enable or close audio
 *
 * @param[in] nPort Port number
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
...
/**
 * Audio/Video sampling data callback function[you can define two function]
 *
...
/**
 * The types of event callback notifications for video capturing.
 */
typedef enum
{
    RECORD_STATUS_NONE = 0,
    RECORD_STATUS_CAMERA_OPEN_SUCCESS,					//Camera opened successfully
    RECORD_STATUS_CAMERA_RUNTIME_BAD_ERROR,				//The camera is running abnormally
    RECORD_STATUS_CAMERA_ENTER_BACKGROUND,				//The program enters the background (IOS will prompt)
    RECORD_STATUS_CAMERA_WILL_ENTER_FOREFROUND,			//The application enters the foreground (IOS will prompt you)
    RECORD_STATUS_ENCORDER_CREATE_FAILED,				//Encoder creation failure
    RECORD_STATUS_ENCORDER_FRAME_FAILED					//Description Failed to insert the encoding frame
}CAMERA_RECORD_NOTIFY;

/**
 * Video capture event callback function.
 *
 * @param[in] notifyType event type
 * @param[in] extraData Reserved additional data pointer
 * @param[in] pUserData userdata
 */
typedef void (CALLBACK *pCameraVideoRecordNotify)(UINT notifyType,void* extraData, void* pUserData);
...
typedef enum
{
	...
	AUDIO_G711A,			// g711a with private head(support 8000 frequency)
	AUDIO_G711U,			// g711u with private head(support 8000 frequency)
	AUDIO_PCM,				// PCM with private head
	AUDIO_G7221_ENC,		// G7221 encode with private head(support 16000 32000 frequency)
	AUDIO_G726_40_ENC,		// G726 40kbps encode with private head(support 8000 frequency)
	AUDIO_G726_32_ENC,		// G726 32kbps encode with private head(support 8000 frequency)
	AUDIO_G726_24_ENC,		// G726 24kbps encode with private head(support 8000 frequency)
	AUDIO_G726_16_ENC,		// G726 16kbps encode with private head(support 8000 frequency)
	AUDIO_AAC_ENC,			// AAC encode with private head, frequency support: 8,11.025,12,16,22.05,24,32,44.1,48,64KHz
	AUDIO_AMR_NB_ENC,		// AMR NB encode with private head(support 8000 frequency)
	AUDIO_CAPTURE_END,
} AUDIO_CAPTURE_TYPE;
 ...
 *
 * @param[in] bStart start speech change,1-enable,0-closed, only support Android/IOS
...
typedef enum
{
    CAMERA_TYPE_BACK = 0,  	//rear camera
    CAMERA_TYPE_FRONT		//front camera
}CAMERA_TYPE;

/**
 * Camera recording parameter structure
 */
typedef struct{
...
/**
 * Capability set query parameter structure
 */
typedef struct{

UINT cameraType;
UINT width;  			//ratio like: 9 : 16
UINT height;

}CAMERA_CAPABILITY_PARAM;

/**
 * Capability set query return structure
 */
typedef struct{

UINT count;				
UINT * arrayPixelWidth;
UINT * arrayPixelHeight;

}CAMERA_RECORD_CAPABILITY;

/**
 * Capture the camera's capability set, which is currently focused on resolution support information
 *
 * @param[in] pData Capability set data block pointer
 * @return void
 */
PLAYSDK_API CAMERA_RECORD_CAPABILITY* CALLMETHOD PLAY_GetCameraCapability(CAMERA_CAPABILITY_PARAM * pParam);

/**
 * Destroy capability set data.
 *
 * @param[in] pData Capability set data block pointer
 * @return void
 */
PLAYSDK_API void CALLMETHOD PLAY_FreeCameraCapability(CAMERA_RECORD_CAPABILITY* pData);

/**
 * Turn on the camera video capture function.
 *
 * @param[in] pProc Video collection data callback pointer
 * @param[in] pNotify Camera, recording status notification callback
 * @param[in] pParam Video recording parameters
 * @param[in] pUserData UserData
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_OpenCameraVideoRecord(pCallFunction pProc, pCameraVideoRecordNotify pNotify, CAMERA_RECORD_PARAM* pParam, void* pUserData);

/**
 * Disable the destroy camera video capture function.
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_CloseCameraVideoRecord();
 ...
 *
 * @param[in] nPort Port number
 * @param[in] nPause 1,Pause,0,Resume
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
 ...
 *
 * @param[in] nPort Port number
 * @param[in] nAVSyncType code stream playing datum. 0,based on video ,1,based on the timestamp
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
 ...
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
PLAYSDK_API float CALLMETHOD PLAY_GetPlaySpeed(LONG nPort);
...
typedef enum
{
	...
	REALTIME_FIRST,			// real-time first
...
typedef enum
{
	...
	FRAME_SUB_TYPE_VIDEO_SMART_P_FRAME = 20, 	// smart P Frame
	FRAME_SUB_TYPE_VIDEO_SMART_I_FRAME_NORENDER = 21, //smart I Frame(NO Render)
 ...
 *
 * @param[in] nPort Port number 
 * @param[in] nEncryptType Encrypt Type  
 * @param[in] szKey Key pointer
 * @param[in] nKeylen Key len
 * @return BOOL, TRUE--succeeded FALSE--failed
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx
 ...
 */
PLAYSDK_API char* CALLMETHOD PLAY_GetDiskSerialNumber(UINT nDiskIndex);

/**
 * Get disk type(platform Windows)
 *
 * @param[in] nDiskIndex disk index.
 * @return int, successful return disk type, 0 is PrivateFS, 1 is XFS; failed return negative number.
 */
...
/**
 * query all files in disk(platform Windows)
 *
 * @param[in] channel NVR channelNo.
 * @param[in] start_time start time for query.
 * @param[in] end_time	 end time for query.
 ...
 * @param[in] type the type of the file of querying
 * @param[in] nDiskIndex disk index
 * @param[in] driver_type drive type
 * @return UINT,TRUE--succeeded FALSE--failed
 */
...
/**
 * enable or close the second fish-eye window in floating mode.
 *
 * @param[in] nPort Port number
 * @param[in] hDestWnd handle of display window
 * @param[in] pOptParam fish-eye params
 * @param[in] bEnable TRUE means enable, FALSE means close
 * @return BOOL,TRUE--succeeded FALSE--failed
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
...
/*
 * Convert YUV data to image memory data.
 *
 * @param[in] pImageInfo image convert struct.
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
typedef struct
{
	unsigned char datatype;	  //input data type,see T_IYUV                                                       */
	unsigned char encodetype; //encryptrype,see ENCRYPT_TYPE 
	char		  reserved[2];			

	unsigned char *inbuf;	  //input data
	unsigned int  inbuf_len;  //input data size

	int width;			      //input width, needed calculate outbuf_len
	int height;		          //input height,needed calculate outbuf_len	

	tPicFormats   to_formats;  //output picture format
	unsigned char *outbuf;     //output data, alloc by user,if NULL get the memory needed with outbuf_len.
	unsigned int  outbuf_len;  //output data size

	unsigned char *vkek;		//use for decrypt vk
 ...
 * @param[in] nRgbType designated RGB format 0:RGB32;1:RGB24;
 * @return BOOL,TRUE--succeeded FALSE--failed.
 * @note if failed,you can get the error code by calling PLAY_GetLastErrorEx.
 */
...
/*
 * Drawing Callback function, use HDC
 * 
 * @param[in] nPort Port number
...
/*
 *Drawing Callback function,use RenderHandle
 * 
 * @param[in] nPort Port number
 * @param[in] void* pRenderHandle render handle
 * @param[in] pUserData Customized Parameter
 */
typedef void (CALLBACK* fDrawCBRenderHandleFun)(LONG nPort,void* pRenderHandle, void* pUserData);

/*
 * Register one callback function to get RenderHandle. Use this API Can use Render engine to Draw something.
 * @param[in] nPort Port number
 * @param[out] DrawFun Callback function handle
 * @param[in] pUserData Customized Parameter
 * @return BOOL,TRUE--succeeded FALSE--failed.
...
typedef enum
{
	...
	DATA_RECORD_FLV,			  // FLV
    DATA_RECORD_DOUBLE_AUDIO_DAV, // Record Double Direct Audios format Dav, audio file maybe small,no support segment.
	DATA_RECORD_ORIGINAL_SEQUENCE,	// Video and Data Frame sort by UTC
	DATA_RECORD_RESIZE_ASF,			// ASF 
	// if need to add enumeration, add before it
...
/*
 * Set Decode&Render Engine, Must be Called before PLAY_Play.
 *
 * @param[in] nPort Port number
...
/*
 * Anti sawtooth enabled switch, platform windows
 *
 * @param[in] nPort Port number
...
//IVS TRACKEX2 Show Text Control 
typedef struct
{
	BOOL            objtype_enable;                // show object type, 0:NO 1:YES
	BOOL            attribute88_enable;            // show 0x88 attribute, 0:NO 1:YES
	BOOL            objid_enable;                  // show object ID, 0:NO 1:YES
	BOOL            age_enable;					   // show age,0:NO 1:YES
}IVSDRAWER_TrackEx2Config;
/**
 *
 * @param[in] nPort Port number
 * @param[in] trackex2config Show Text Control
 */
...
typedef enum _IVS_TYPE
{
	...
	IVSINFOTYPE_TRAFFIC_LIGHT		= 23,		    // traffic light(0x1D)
	IVSINFOTYPE_PTZ_LOCATION		= 24,			// PTZ location(0x21)
	IVSINFOTYPE_DATA_WITH_VIDEO_INFO_OVERLAY = 25, //video Info overlay(0x23)
}IVS_TYPE;
...
/* IVSINFOTYPE_TRAFFIC_LIGHT struct */
typedef struct 
{
	unsigned char	version;    // version
	unsigned char	redLight;   // red light
	unsigned char	yellowLight;// yellow light
	unsigned char	greenLight;	// green light
...
typedef enum _STATISTIC_TYPE
{
	...
	INPUT_DATA_INTERVAL,					// Input stream data interval, parameter 1 means time interval, parameter 2 means data length
	PRASE_VIDEO_INTERVAL,					// Video frame resolution interval, parameter 1 means time interval,parameter 2 means frame number
	VIDEO_PTS_INTERVAL,						// Video frame pts interval, parameter 1 means time interval, parameter 2 means frame number
	DECODE_VIDEO_TIME,						// Video frame decoding time consuming, parameter 1 means interface time consuming, parameter 2 means frame number
	PLAY_VIDEO_INTERVAL,					// Video frame playing interval, parameter 1 means time interval, parameter 2 means frame number
	RENDER_VIDEO_TIME,						// Video frame rendering time consuming, parameter 1 means interface time consuming
	VIDEO_DECODE_ERROR = 1000,		// Video decoding error, parameter 1 lack of meaning, parameter 2 means frame number
...
typedef enum
{
	AUDIO_PLAYBACK_MODE_VOICE = 0,			//voice mode
	AUDIO_PLAYBACK_MODE_SYSTEM,				//system mode
	AUDIO_PLAYBACK_MODE_RING,				//ring mode
	AUDIO_PLAYBACK_MODE_MEDIA,				//media mode
	AUDIO_PLAYBACK_MODE_ALARM,				//alarm mode
	AUDIO_PLAYBACK_MODE_NOTIFICATION,		//notification mode
}AUDIO_PLAYBACK_MODE;

/*
 * Set audio playback mode(only support Android)
 *
 * @param[in] nPort Port number
 * @param[in] nMode Audio playback mode
 * @return BOOL,TRUE--succeeded FALSE--failed.
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetAudioPlaybackMode(LONG nPort, AUDIO_PLAYBACK_MODE nMode);

/*
 * @brief set window size when changed.
 * @param[in] nPort Port number
 * @param[in] nWidth window width
 * @param[in] nHeight window height
 * @param[in] nRegionNum region num default 0
 * @return BOOL,TRUE--succeeded FALSE--failed.
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_ViewResolutionChanged(LONG nPort, int nWidth, int nHeight, DWORD nRegionNum);

/*
 * enable audio noise elimination
 * @param nPort Port number
 * @param nMode value is: -1,0,1,2,3,4  
 *				-1:  noise elimination is off
 *				0:   The degree of noise elimination is the lowest, and the damage to useful speech signals is minimal
 *				4:   The degree of noise elimination is the greatest, and the damage to useful speech signals is greatest
...
*
* @param[in] languageType, Chinese or English
*/
typedef enum
{
	PLAY_LANGUAGE_CHN = 0, //Chinese
	PLAY_LANGUAGE_ENG,     //English
}PLAY_LANGUAGE_TYPE;
PLAYSDK_API BOOL CALLMETHOD PLAY_SetLanguageEnvi(PLAY_LANGUAGE_TYPE languageType);
